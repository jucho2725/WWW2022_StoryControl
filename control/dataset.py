...
def load_and_cache_examples_train(data_args, tokenizer):
    ...
    def preprocess_function(examples):
        ...
        model_inputs = tokenizer(inputs_09, add_special_tokens=True,
                                    truncation=True, padding=padding, max_length=max_source_length)
        model_inputs_final['aug_09']['input_ids'] = tokenizer.encode(genre, add_prefix_space=True) + model_inputs['input_ids']
        model_inputs_final['aug_09']['attention_mask'] = [1] + model_inputs['attention_mask']
        # augmented input en-ru
        model_inputs = tokenizer(inputs_05, add_special_tokens=True,
                                    truncation=True, padding=padding, max_length=max_source_length)
        model_inputs_final['aug_05']['input_ids'] = tokenizer.encode(genre, add_prefix_space=True) + model_inputs['input_ids']
        model_inputs_final['aug_05']['attention_mask'] = [1] + model_inputs['attention_mask']
